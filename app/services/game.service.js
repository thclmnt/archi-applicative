...
const GameService = {
    ...
    choices: {
        findCombinations: (dices, isDefi, isSec) => {
            ...
            let isLessThanEqual8 = false; // Pour vérifier si la somme des dés est inférieure ou égale à 8
            ...
            if (isDefi && availableCombinations.length > 0 && notOnlyBrelan) {
        ...
        selectCell: (idCell, rowIndex, cellIndex, currentTurn, grid) => {
            ...
            for (let i = 0; i < updatedGrid.length; i++) {
                for (let j = 0; j < updatedGrid[i].length; j++) {
                    if (i === rowIndex && j === cellIndex && updatedGrid[i][j].id === idCell) {
                        updatedGrid[i][j].owner = currentTurn;
                    }
                }
            }
    ...
    score : {
        calculateScoreAndWinner: (grid) => {
            ...
            const checkPointsAlignment = (points) => {
